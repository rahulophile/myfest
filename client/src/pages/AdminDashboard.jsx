...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [eventsResponse, registrationsResponse] = await Promise.all([
        ...
        fetch(buildApiUrl("/api/admin/registrations"), {
  ...
  const checkDatabaseState = async () => {
    try {
      ...
      const response = await fetch(
        buildApiUrl("/api/admin/debug/database-state"),
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const filteredRegistrations = useMemo(() => {
    return registrations.filter((reg) => {
      ...
      const userName = reg.userId?.name || "";
      const userIdStr = reg.userId?.userId || "";
      const teamName = reg.teamName || "";

      const matchesEvent =
        selectedEvent === "all" || eventTitle === selectedEvent;
      const q = searchQuery.toLowerCase();
      const matchesSearch =
        q === "" ||
        userIdStr.toLowerCase().includes(q) ||
  ...
  const stats = useMemo(() => {
    return {
      ...
      uniqueUsers: new Set(registrations.map((r) => r.userId?._id)).size,
      totalTeams: new Set(
        registrations.filter((r) => r.teamName).map((r) => r.teamName)
      ).size,
    };
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-300 text-sm">
                Vision Fest 25 - Event Management
              </p>
            </div>
            ...
            <div className="bg-gray-800 rounded-lg p-6 mb-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="Search by User ID, Team, or Name"
                    value={searchQuery}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            <div className="text-sm font-medium text-white">
                              {registration.userId?.name || "N/A"}
                            </div>
                            <div className="text-sm text-cyan-400 font-mono">
                              {registration.userId?.userId || "N/A"}
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-white">
                            {registration.eventId?.title || "Unknown"}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-xs text-gray-400">
                            {registration.userId?.collegeName || "N/A"}
                          </div>
                        </td>
          ...
          <div className="space-y-6">
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              <p className="text-gray-400">
                Select an event to view its registered teams.
              </p>
            </div>
            <div className="bg-gray-800 rounded-lg p-6">
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6">
      <div className="flex items-center">
        <div className={`p-2 ${colors[color]} rounded-lg`}>{icon}</div>
        <div className="ml-4">
